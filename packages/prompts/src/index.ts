...
export function generateStubResponse(
  ...
  switch (jobType) {
    ...
    case 'deck_content':
      return generateDeckContentStub(context, date);
    default:
...
function generateDeckContentStub(context: PromptContext, date: string): string {
  const topic = (context.topic as string) || 'Q4 Product Update';
  const audienceType = (context.audienceType as string) || 'exec';
  const audienceLabel = audienceType === 'customer' ? 'Customer' 
    : audienceType === 'team' ? 'Team'
    : audienceType === 'stakeholder' ? 'Stakeholder'
    : 'Executive';
  
  return `# Slide Deck Content: ${topic}

**Audience**: ${audienceLabel}
**Generated**: ${date}
**Prepared for**: ${context.userName || 'Product Team'}
...
**Speaker Notes:** Open with energy. This deck covers our progress, impact, and what's next. Keep it conversational—this is a discussion, not a lecture.
...
**Speaker Notes:** Set expectations upfront. Let them know there will be time for questions at the end, but they can interrupt for clarifications.
...
**Speaker Notes:** Pause here to let the numbers sink in. This is the "why now" moment. If asked about competitors, mention Notion's recent AI search launch.
...
**Speaker Notes:** Demo opportunity here if time allows. Show the before/after experience. Emphasize this was customer-driven, not a pet project.
...
**Speaker Notes:** These are beta numbers from 200 users. Full rollout metrics expected in 2 weeks. Globex Corp specifically called out filters as "game-changing."
...
**Speaker Notes:** These are real quotes from beta users. Globex is now moving forward with their 4x expansion. Don't oversell—acknowledge we still have work to do on AI search.
...
**Speaker Notes:** AI search is the big bet for Q1. We're starting discovery now. SSO is running in parallel—that's a separate update if needed.
...
**Speaker Notes:** Be direct about the ask. We need a decision on AI search resourcing by end of week to hit Q1 targets. Open for questions.
...
**Likely Questions:**
1. "Why not AI search first?" → Customer evidence showed filters were table-stakes; AI search is differentiation
...
*Generated by pmkit • Deck Content v1*`;
}
...
export function createStubGenerator(): (messages: LLMMessage[]) => string {
  return (messages: LLMMessage[]) => {
    ...
    } else if (systemPrompt.includes('slide content') || systemPrompt.includes('presentation content') || systemPrompt.includes('deck content')) {
      return generateStubResponse('deck_content', context);
    }
